...
class Protocol(threading.local):
    ...
    MAGIC = {'request': 0x80, 'response': 0x81}
    ...
    COMMANDS = {
        ...
        'mult': {'command': 0x1F, 'struct': 'QQL%ds'},
        'multq': {'command': 0x2F, 'struct': 'QQL%ds'},
    ...
    def _send_authentication(self):
        ...
        self._send(
            struct.pack(
                self.HEADER_STRUCT + self.COMMANDS['auth_request']['struct'] % (len(method), len(auth)),
                self.MAGIC['request'],
                self.COMMANDS['auth_request']['command'],
                len(method),
                0,
                0,
                0,
    ...
    def get(self, key):
        ...
        data = struct.pack(
            self.HEADER_STRUCT + self.COMMANDS['get']['struct'] % (len(keybytes),),
            self.MAGIC['request'],
    ...
    def noop(self):
        ...
        data = struct.pack(
            self.HEADER_STRUCT + self.COMMANDS['noop']['struct'],
            self.MAGIC['request'],
    ...
    def get_multi(self, keys):
        ...
        while opcode != self.COMMANDS['getk']['command']:
            ...
            if status == self.STATUS['success']:
                flags, key, value = struct.unpack('!L%ds%ds' % (keylen, bodylen - keylen - 4), extra_content)
    ...
    def _set_add_replace(self, command, key, value, time, cas=0, compress_level=-1):
        ...
        self._send(
            struct.pack(
                self.HEADER_STRUCT + self.COMMANDS[command]['struct'] % (len(keybytes), len(value)),
                self.MAGIC['request'],
    ...
    def _incr_decr(self, command, key, value, default, time):
        ...
        self._send(
            struct.pack(
                self.HEADER_STRUCT + self.COMMANDS[command]['struct'] % len(key),
                self.MAGIC['request'],
                self.COMMANDS[command]['command'],
                len(keybytes),
                20,
    ...
    def mult(self, key, value, default=0, time=1000000):
        """
        Multiply a key, if it exists, returns its actual value, if it doesn't, return 0.
        ...
        :rtype: int
        """
        return self._incr_decr('mult', key, value, default, time)
    ...
    def delete(self, key, cas=0):
        ...
        self._send(
            struct.pack(
                self.HEADER_STRUCT + self.COMMANDS['delete']['struct'] % (len(keybytes),),
                self.MAGIC['request'],
    ...
    def flush_all(self, time):
        ...
        self._send(
            struct.pack(
                self.HEADER_STRUCT + self.COMMANDS['flush']['struct'],
                self.MAGIC['request'],
    ...
    def stats(self, key=None):
        ...
        if key is not None:
            ...
            packed = struct.pack(
                self.HEADER_STRUCT, self.MAGIC['request'], self.COMMANDS['stat']['command'], 0, 0, 0, 0, 0, 0, 0
            )
