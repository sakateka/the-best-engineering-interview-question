...
class Client(threading.local):
    ...
    def delete(self, key, time=None, noreply=False):
        ...
        @rtype: int
        '''
        return self._deletetouch([b'DELETED', b'NOT_FOUND'], "delete", key, time, noreply)
    ...
    def _deletetouch(self, expected, cmd, key, time=0, noreply=False):
        ...
        try:
            ...
            if line and line.strip() in expected:
                return 1
            self.debuglog('%s expected %s, got: %r' % (cmd, b' or '.join(expected), line))
        except socket.error as msg:
            if isinstance(msg, tuple):
                msg = msg[1]
            server.mark_dead(msg)
    ...
    def mult(self, key, delta=1, noreply=False):
        """Multiply value for C{key} by C{delta}

        Sends a command to the server to atomically multiply the
        value for C{key} by C{delta}, or by 1 if C{delta} is
        unspecified.  Returns None if C{key} doesn't exist on server,
        otherwise it returns the new value after multipying.
        ...
        >>> mc.set("counter", "20")  # returns 1, indicating success
        1
        >>> mc.mult("counter", delta=3)
        60
        >>> mc.mult("counter", delta=2)
        120

        Overflow on server is not checked.  Be aware of values
        approaching 2**32.  See L{decr}.

        @param delta: Integer amount to multipy by (should be zero
        or greater).

        @param noreply: optional parameter instructs the server to not send the
        reply.

        @return: New value after multipying, no None for noreply or error.
        @rtype: int
        """
        return self._incrdecr("mult", key, delta, noreply)
    ...
    def _map_and_prefix_keys(self, key_iterable, key_prefix):
        ...
        for orig_key in key_iterable:
            if isinstance(orig_key, tuple):
                ...
                server, key = self._get_server((serverhash, key_prefix + key))
    ...
    def set_multi(self, mapping, time=0, key_prefix='', min_compress_len=0, noreply=False):
        ...
        server_keys, prefixed_to_orig_key = self._map_and_prefix_keys(six.iterkeys(mapping), key_prefix)
            ...
            try:
                for key in server_keys[server]:  # These are mangled keys
                    ...
                    if store_info:
                        ...
                        fullcmd = self._encode_cmd('set', key, headers, noreply, b'\r\n', val, b'\r\n')
                        write(fullcmd)
                    else:
    ...
    def _set(self, cmd, key, val, time, min_compress_len=0, noreply=False):
        ...
        def _unsafe_set():
            ...
            if cmd == 'cas':
                headers = "%d %d %d %d" % (flags, time, len_val, self.cas_ids[key])
            else:
                headers = "%d %d %d" % (flags, time, len_val)
    ...
    def check_key(self, key, key_extra_len=0):
        ...
        if not valid_key_chars_re.match(key):
            raise Client.MemcachedKeyCharacterError("Control/space characters not allowed (key=%r)" % key)
...
class _Host(object):
    def __init__(self, host, debug=0, dead_retry=_DEAD_RETRY, socket_timeout=_SOCKET_TIMEOUT, flush_on_reconnect=0):
        ...
        if not m:
            m = re.match(r'^(?P<proto>inet6):' r'\[(?P<host>[^\[\]]+)\](:(?P<port>[0-9]+))?$', host)
        if not m:
            m = re.match(r'^(?P<proto>inet):' r'(?P<host>[^:]+)(:(?P<port>[0-9]+))?$', host)
        if not m:
    ...
    def expect(self, text, raise_exception=False):
        ...
        if self.debug and line != text:
            if six.PY3:
                ...
                log_line = line
            self.debuglog("while expecting %r, got unexpected response %r" % (text, log_line))
        return line
